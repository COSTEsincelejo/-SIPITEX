...
public class MigrationBaselineTests
{
    ...
    public async Task NewDatabase_MigrateAsync_CreatesFullSchema()
    {
        ...
        try
        {
            ...
            // ... + AddBomProductAndOrderSnapshot + AddActaVerificacion = 11
            ...
            Assert.True(await TableExistsAsync(dbPath, "ActasVerificacion"));
        }
        finally
        {
    ...
    public async Task LegacyFullSchemaWithoutHistory_BaselineThenMigrate_Succeeds()
    {
        ...
        try
        {
            ...
            await using (var conn = new SqliteConnection($"Data Source={dbPath}"))
            {
                ...
                Assert.Contains(ids, id => id.Contains("AddActaVerificacion", StringComparison.Ordinal));
            }
        }
        finally
        {
