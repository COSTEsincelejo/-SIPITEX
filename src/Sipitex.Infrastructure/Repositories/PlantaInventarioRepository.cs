...
public class PlantaInventarioRepository : IPlantaInventarioRepository
{
    ...
    public async Task<IReadOnlyList<PlantaInventario>> GetAllAsync(CancellationToken cancellationToken = default) =>
        await _context.PlantasInventario.AsNoTracking().OrderBy(p => p.Nombre).ToListAsync(cancellationToken);

    public Task<PlantaInventario?> GetByIdAsync(int id, CancellationToken cancellationToken = default) =>
        _context.PlantasInventario.FirstOrDefaultAsync(p => p.Id == id, cancellationToken);
        ...
        int? excludeId = null)
    {
        ...
        var query = _context.PlantasInventario.AsQueryable().Where(p => p.Nombre.ToLower() == normalized);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var encargados = await _context.UserPlantasInventario.CountAsync(up => up.PlantaInventarioId == plantaInventarioId, cancellationToken);
        return new PlantaInventarioDependencias(materiales, solicitudes, encargados);
    }

    public async Task AddAsync(PlantaInventario planta, CancellationToken cancellationToken = default) =>
        await _context.PlantasInventario.AddAsync(planta, cancellationToken);

    public void Update(PlantaInventario planta) => _context.PlantasInventario.Update(planta);

    public void Remove(PlantaInventario planta) => _context.PlantasInventario.Remove(planta);
}
