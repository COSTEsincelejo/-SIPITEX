...
public class SolicitudMaterialService : ISolicitudMaterialService
{
        ...
        CancellationToken cancellationToken)
    {
        if (dto.FichaId is not int fichaId || fichaId <= 0)
            return ServiceResult.Fail("La ficha SENA es obligatoria para solicitudes por grupo.");
        ...
        var bodegaIds = materiales.Select(m => m.PlantaInventarioId).Distinct().ToList();
        // Una SolicitudMaterial tiene un solo PlantaInventarioId. Si los materiales son de plantas distintas
        // se rechaza aunque el encargado de bodega actual tenga todas esas plantas asignadas.
        ...
        await NotifyEncargadosDePlantaAsync(
            bodegaId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Sin MaterialId aún: el solicitante puede pasar PlantaInventarioId; si no, Planta 1 (backfill).
        var bodegaId = dto.PlantaInventarioId is > 0 ? dto.PlantaInventarioId.Value : DefaultPlantaInventarioId;
        var bodega = await _plantaInventarioRepository.GetByIdAsync(bodegaId, cancellationToken);
        if (bodega is null)
    ...
    private async Task NotifyEncargadosDePlantaAsync(
        int bodegaId,
        AlertType type,
        string subject,
        string body,
        CancellationToken cancellationToken)
    {
        var ids = (await _userRepository.GetAllAsync(cancellationToken))
            .Where(u => u.IsActive
                        && string.Equals(u.Rol, UserRoles.EncargadoBodega, StringComparison.OrdinalIgnoreCase)
                        && u.UserPlantasInventario.Any(ub => ub.PlantaInventarioId == bodegaId))
            .Select(u => u.Id)
            .ToList();
