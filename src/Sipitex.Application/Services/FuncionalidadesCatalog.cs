...
public static class FuncionalidadesCatalog
{
    ...
    [
        ...
        new("Inventario", "Consultar materiales",
            ...
            $"{UserRoles.Administrador}, {UserRoles.EncargadoBodega}, {UserRoles.Instructor}"),
        new("Inventario", "Consultar historial de movimientos",
            "Lista entradas, salidas, ajustes y aprobaciones con fecha, usuario y cantidad.",
            $"{UserRoles.Administrador}, {UserRoles.EncargadoBodega}"),
        new("Inventario", "Registrar material",
            "Crea un material nuevo en bodega (nombre, unidad, stock, origen de entrada).",
            $"{UserRoles.Administrador}, {UserRoles.EncargadoBodega} (+ permiso extendido)"),
        ...
        new("MRP / Materiales", "Crear / editar producto BOM",
            ...
            $"{UserRoles.Administrador}, {UserRoles.EncargadoBodega} (+ permiso extendido Mrp.GestionarFichas)"),
        ...
        // --- Bodega ---
        new("Planta de inventario — solicitudes", "Cola de solicitudes",
            "Lista todas las solicitudes de material pendientes o resueltas.",
            UserRoles.EncargadoBodega),
        new("Planta de inventario — solicitudes", "Resolver solicitud",
            "Aprueba o rechaza una solicitud y genera código de entrega.",
            UserRoles.EncargadoBodega),
        new("Planta de inventario — órdenes", "Reingreso desde etapas",
        ...
        new("Estadísticas", "Dashboard KPI",
            ...
            $"{UserRoles.Administrador}, {UserRoles.Instructor}, {UserRoles.EncargadoBodega}"),
        ...
        new("Administración", "Asignar roles y permisos",
            "Define rol (Administrador/Instructor/Encargado de bodega), ficha, planta de inventario (si es Encargado de bodega) y permisos extendidos.",
            UserRoles.Administrador),
        new("Administración", "Gestionar bodegas",
            "Lista y crea bodegas físicas. Un bodeguero debe quedar asignado a una bodega para ver su cola de solicitudes.",
            UserRoles.Administrador),
        new("Administración", "Consultar auditoría",
            "Bitácora de quién hizo qué en acciones críticas: usuarios (incl. bodegas asignadas), bodegas, órdenes de producción y fichas técnicas BOM.",
            UserRoles.Administrador),
