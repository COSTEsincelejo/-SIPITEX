...
public class UserAccountService : IUserAccountService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var resolvedPlanta = await ResolvePlantaInventarioIdsAsync(rol, plantaInventarioIds, cancellationToken);
        if (!resolvedPlanta.Success)
            return ServiceResult.Fail(resolvedPlanta.Message ?? "Planta de inventario no válida.");
    ...
    // EncargadoDeBodega exige ≥1 plantaInventario existente; otros roles ignoran el valor y quedan sin asignaciones.
    ...
    {
        if (!string.Equals(rol, UserRoles.EncargadoDeBodega, StringComparison.OrdinalIgnoreCase))
            return (true, null, []);
    ...
    private static void ReplaceUserPlantasInventario(User user, IReadOnlyList<int> plantaInventarioIds)
    {
        ...
        var toRemove = user.UserPlantasInventario.Where(ub => !desired.Contains(ub.PlantaInventarioId)).ToList();
        foreach (var row in toRemove)
            user.UserPlantasInventario.Remove(row);

        var existing = user.UserPlantasInventario.Select(ub => ub.PlantaInventarioId).ToHashSet();
