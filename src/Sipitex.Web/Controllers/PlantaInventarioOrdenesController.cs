...
public class PlantaInventarioOrdenesController : Controller
{
    ...
    public PlantaInventarioOrdenesController(
    ...
    {
        return View(new PlantaInventarioOrdenesIndexViewModel
        {
    ...
    {
        if (!TryGetActor(out var encargadoBodegaId, out var nombre))
        {
    ...
    {
        if (!int.TryParse(User.FindFirstValue(ClaimTypes.NameIdentifier), out var encargadoBodegaId))
        {
            TempData["Message"] = "Sesión de encargado de bodega no válida.";
            TempData["IsSuccess"] = false;
            return RedirectToAction(nameof(Index));
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (orderId is int oid and > 0)
        {
            ...
            var mes = await _flowService.GetMesDetailAsync(oid, uid, UserRoles.EncargadoBodega, cancellationToken);
            if (mes is not null)
            {
                stages = mes.Stages
