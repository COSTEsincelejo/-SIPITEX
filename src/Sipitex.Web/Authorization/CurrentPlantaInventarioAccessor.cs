...
// Lee planta_inventario_id (0..N claims) emitidos en el login. Sin IUserRepository para no ciclar con el DbContext.
...
public sealed class CurrentPlantaInventarioAccessor : ICurrentPlantaInventarioAccessor
{
    ...
    public IReadOnlyList<int>? PlantaInventarioIds
    {
        get
        {
            ...
            if (!user.IsInRole(UserRoles.EncargadoBodega))
                return null;
